...
struct PeaksSpectrogram: View {
    ...
    var body: some View {
        ZStack {
            if (settings.option == 0 || settings.option == 1) {           // top-to-bottom flow
...
fileprivate struct PeaksSpectrogram_Live: View {
    @EnvironmentObject var manager: AudioManager        // Observe the instance of AudioManager passed from ContentView
    @EnvironmentObject var settings: Settings
    ...
    var body: some View {
        ...
        Canvas { context, size in
            ...
            if (option == 0 || option == 1) {                       // vertical scrolling from top to bottom
